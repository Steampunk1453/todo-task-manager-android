...
internal class CreateBookTest {
    ...
    @InjectMockKs
    private lateinit var useCase: CreateBook

    @Test
    fun `should return book when execute create`() = runBlockingTest  {
        // Given
        val book = BookStub.random()
        coEvery { repository.save(any()) } returns Result.Success(book)
        // When
    ...
    @Test
    fun `should return null book when execute create`() = runBlockingTest  {
        // Given
        val book = BookStub.random()
